...
void Mesh::CreateMesh(GLfloat *vertices, unsigned int *indices, unsigned int numOfVertices, unsigned int numOfIndices)
{
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, 0);	// this is where we explain how <vertices> is structured (see documentation if you dont understand)
	...
	// same as above, now we pass/explain texture data for each triangle
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, (void*) (sizeof(vertices[0]) * 3));
	glEnableVertexAttribArray(1);

	// same as above, now we pass/explain normal data for each triangle
